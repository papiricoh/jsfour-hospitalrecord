...
Citizen.CreateThread(function()
	...
	TriggerEvent('chat:addSuggestion', '/pdaems', ' Terminal de Datos movil EMS', {})	
	end
...
RegisterCommand('pdaems',function()

if (ESX.PlayerData.job.name == 'ambulance') then


        if (holdingTablet==true) then
            TriggerEvent("htabletmdttemporal")
            print ("AnimacionTablet")
            print (holdingTablet)
            holdingTablet=false

ESX.TriggerServerCallback('jsfour-hospitalrecord:fetch', function(d)
...
SetNuiFocus(true, true)
	...
	exports['mythic_notify']:DoCustomHudText('cajaroja', ' No eres e o estas en servicio para usar este comando ',3500)
end
...
RegisterNetEvent("htabletmdttemporal")
AddEventHandler("htabletmdttemporal", function()
...
RegisterNUICallback('search', function(data, cb)
	ESX.TriggerServerCallback('jsfour-hospitalrecord:search', function( d )
    cb(d)
  end, data)
end)
...
RegisterNUICallback('add', function(data, cb)
	ESX.TriggerServerCallback('jsfour-hospitalrecord:add', function( d )
    cb(d)
  end, data)
end)
...
RegisterNUICallback('update', function(data, cb)
	ESX.TriggerServerCallback('jsfour-hospitalrecord:update', function( d )
    cb(d)
  end, data)
end)
...
RegisterNUICallback('remove', function(data, cb)
	ESX.TriggerServerCallback('jsfour-hospitalrecord:remove', function( d )
    cb(d)
  end, data)
end)
